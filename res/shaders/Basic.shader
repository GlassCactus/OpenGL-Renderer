...
vec3 PointLights(PointLight light, vec3 FragPos)
{
	...
	//ambient
	vec3 ambience = lightCol * box.ambientCol;// texture(material.diffuse, Tex).rgb;

	//diffuse
	vec3 lightDir = normalize(light.position - FragPos);
	float diff = max(dot(Normal, lightDir), 0.0f);
	vec3 diffuse = lightCol * diff * box.diffCol;// texture(material.diffuse, Tex).rgb;
	...
	float spec = max(dot(Normal, halfwayDir), 0.0f);
	spec = pow(spec, box.alpha) * ((box.alpha + 2.0) / (4.0 * PI * (2.0 - exp(-box.alpha / 2.0))));
	vec3 specular = spec * (lightCol*0.25) * box.specCol;//texture(material.specular, Tex).rgb;
...
{
	material;
	vec3 phong = (pow(PointLights(pointlight, FragPos).rgb, vec3(1.0 / GAMMA)));
